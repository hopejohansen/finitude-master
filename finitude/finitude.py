...
class HvacMonitor:
    FRAME_COUNT = prometheus_client.Counter('finitude_frames',
                                            ...
                                            # In metrics.py labels() this array is compared to kwargs to be sure that it is the same length.
                                            # kwargs contains 1 value (name), so I'm changing this array to match - HWJ
                                             ['name', 'source', 'dest', 'func', 'register'])  # <-- original code
                                            # ['name'])
